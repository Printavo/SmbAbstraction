    ...
    public static void HandleStatus(this NTStatus status)
    {
        switch (status)
        {
            ...
            case (NTStatus.STATUS_NOT_IMPLEMENTED):
                throw new NotImplementedException($"{status.ToString()}: {c_errBadFunc}");
            case (NTStatus.STATUS_INVALID_DEVICE_REQUEST):
                throw new InvalidOperationException($"{status.ToString()}: {c_errBadFunc}");
            case (NTStatus.STATUS_NO_SUCH_FILE):
            case (NTStatus.STATUS_NO_SUCH_DEVICE):
            case (NTStatus.STATUS_OBJECT_NAME_NOT_FOUND):
                throw new FileNotFoundException($"{status.ToString()}: {c_errBadFile}");
            ...
            case (NTStatus.STATUS_OBJECT_PATH_SYNTAX_BAD):
                throw new DirectoryNotFoundException($"{status.ToString()}: {c_errBadPath}");
            case (NTStatus.STATUS_TOO_MANY_OPENED_FILES):
                throw new FileNotFoundException($"{status.ToString()}: {c_errNoFids}");
            ...
            case (NTStatus.STATUS_CANNOT_DELETE):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_errNoAccess}");
            case (NTStatus.STATUS_SMB_BAD_FID):
            case (NTStatus.STATUS_INVALID_HANDLE):
            case (NTStatus.STATUS_FILE_CLOSED):
                throw new ArgumentException($"{status.ToString()}: {c_errBadFid}");
            case (NTStatus.STATUS_INSUFF_SERVER_RESOURCES):
                throw new OutOfMemoryException($"{status.ToString()}:{c_errNoMem}");
            case (NTStatus.STATUS_OS2_INVALID_ACCESS):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_errBadAccess}");
            case (NTStatus.STATUS_DATA_ERROR):
                throw new InvalidDataException($"{status.ToString()}: {c_errBadData}");
            case (NTStatus.STATUS_DIRECTORY_NOT_EMPTY):
                throw new IOException($"{status.ToString()}: {c_errRemCd}");
            case (NTStatus.STATUS_NO_MORE_FILES):
                throw new IOException($"{status.ToString()}: {c_errNoFiles}");
            case (NTStatus.STATUS_END_OF_FILE):
                throw new IOException($"{status.ToString()}: {c_errEof}");
            case (NTStatus.STATUS_NOT_SUPPORTED):
                throw new NotSupportedException($"{status.ToString()}: {c_errUnsup}");
            case (NTStatus.STATUS_OBJECT_NAME_COLLISION):
                throw new IOException($"{status.ToString()}: {c_errFileExists}");
            case (NTStatus.STATUS_INVALID_PARAMETER):
                throw new ArgumentException($"{status.ToString()}: {c_errInvalidParam}");
            case (NTStatus.STATUS_OS2_INVALID_LEVEL):
                throw new UnsupportedInformationLevelException($"{status.ToString()}: {c_errUnknownLevel}");
            case (NTStatus.STATUS_RANGE_NOT_LOCKED):
                throw new AccessViolationException($"{status.ToString()}: {c_errorNotLocked}");
            case (NTStatus.STATUS_OS2_NO_MORE_SIDS):
                throw new InvalidOperationException($"{status.ToString()}: {c_errorNoMoreSearchHandles}");
            case (NTStatus.STATUS_INVALID_INFO_CLASS):
                throw new UnsupportedInformationLevelException($"{status.ToString()}: {c_errBadPipe}");
            case (NTStatus.STATUS_BUFFER_OVERFLOW):
            case (NTStatus.STATUS_MORE_PROCESSING_REQUIRED):
                throw new InvalidOperationException($"{status.ToString()}: {c_errMoreData}");
            case (NTStatus.STATUS_NOTIFY_ENUM_DIR):
                throw new AccessViolationException($"{status.ToString()}: {c_errNotifyEnumDir}");

            //ERRSRV Class 0x02
            case (NTStatus.STATUS_INVALID_SMB):
                throw new ArgumentException($"{status.ToString()}: {c_errInvSmb}"); //Is there a better exception for this?
            case (NTStatus.STATUS_NETWORK_NAME_DELETED):
            case (NTStatus.STATUS_SMB_BAD_TID):
                throw new ArgumentException($"{status.ToString()}: {c_errInvTid}");
            case (NTStatus.STATUS_BAD_NETWORK_NAME):
                throw new ArgumentException($"{status.ToString()}: {c_errInvNetName}");
            case (NTStatus.STATUS_SMB_BAD_COMMAND):
                throw new NotImplementedException($"{status.ToString()}: {c_errBadCmd}");
            case (NTStatus.STATUS_TOO_MANY_SESSIONS):
                throw new ApplicationException($"{status.ToString()}: {c_errTooManyUids}");
            case (NTStatus.STATUS_ACCOUNT_DISABLED):
            case (NTStatus.STATUS_ACCOUNT_EXPIRED):
                throw new AuthenticationException($"{status.ToString()}: {c_errAccountExpired}");
            case (NTStatus.STATUS_INVALID_WORKSTATION):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_errBadClient}");
            case (NTStatus.STATUS_INVALID_LOGON_HOURS):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_errBadLogonTime}");
            case (NTStatus.STATUS_PASSWORD_EXPIRED):
            case (NTStatus.STATUS_PASSWORD_MUST_CHANGE):
                throw new InvalidCredentialException($"{status.ToString()}: {c_errPasswordExpired}");

            //ERRHRD Class 0x03
            case (NTStatus.STATUS_MEDIA_WRITE_PROTECTED):
                throw new AccessViolationException($"{status.ToString()}: {c_errNoWrite}");
            case (NTStatus.STATUS_SHARING_VIOLATION):
                throw new InvalidOperationException($"{status.ToString()}: {c_errBadShare}");
            case (NTStatus.STATUS_FILE_LOCK_CONFLICT):
                throw new InvalidOperationException($"{status.ToString()}: {c_errLock}");
            ...
            case (NTStatus.STATUS_LOGON_TYPE_NOT_GRANTED):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_ntStatusStatusLogonTypeNotGranted}"); 
            case (NTStatus.STATUS_ACCOUNT_LOCKED_OUT):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_ntStatusStatusAccountLockedOut}");
            case (NTStatus.STATUS_ACCOUNT_RESTRICTION):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_ntStatusStatusAccountRestriction}");
            ...
            case (NTStatus.STATUS_OBJECT_NAME_INVALID):
                throw new MemberAccessException($"{status.ToString()}: {c_ntStatusStatusObjectNameInvalid}");
            case (NTStatus.STATUS_OBJECT_NAME_EXISTS):
                throw new InvalidOperationException($"{status.ToString()}: {c_ntStatusStatusObjectNameExists}");
            case (NTStatus.STATUS_LOCK_NOT_GRANTED):
                throw new IOException($"{status.ToString()}: {c_ntStatusStatusLockNotGranted}");
            case (NTStatus.STATUS_BUFFER_TOO_SMALL):
                throw new ArgumentException($"{status.ToString()}: {c_ntStatusStatusBufferTooSmall}");
            case (NTStatus.STATUS_BAD_DEVICE_TYPE):
                throw new InvalidOperationException($"{status.ToString()}: {c_ntStatusStatusBadDeviceType}");
            case (NTStatus.STATUS_FS_DRIVER_REQUIRED):
                throw new FileLoadException($"{status.ToString()}: {c_ntStatusStatusFsDriverRequired}");
            case (NTStatus.STATUS_USER_SESSION_DELETED):
                throw new UnauthorizedAccessException($"{status.ToString()}: {c_ntStatusStatusUserSessionDeleted}");
            case (NTStatus.SEC_I_CONTINUE_NEEDED):
                throw new InvalidOperationException($"{status.ToString()}");
            case (NTStatus.STATUS_CANCELLED):
                throw new IOException($"{status.ToString()}: {c_ntStatusStatusCancelled}");
            case (NTStatus.STATUS_PENDING):
                throw new InvalidOperationException($"{status.ToString()}: {c_ntStatusStatusPending}");
